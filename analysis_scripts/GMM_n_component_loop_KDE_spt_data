...
with threadpool_limits(limits=1):
    ...
    file_path = '/lcrc/group/ATLAS/users/jj/data/spt/1g3.csv'
    ...
    z_scores = np.abs((random_row_scaled - np.mean(random_row_scaled)) / np.std(random_row_scaled))
    random_row_scaled = random_row_scaled[z_scores < 3]
    ...
    bw_method_ratio = 1
    bw_method = 'silverman'
    kde = gaussian_kde(random_row_scaled, bw_method=bw_method)
