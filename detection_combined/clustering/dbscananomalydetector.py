...
class HLTDBSCANAnomalyDetector(BaseClusteringDetector):
    ...
    def process(self,
        ...
        features = np.array(list(zip(rack_labels_filtered, data_filtered)))
    
        # Apply StandardScaler to scale the features
        scaler = StandardScaler()
        scaled_features = scaler.fit_transform(features)

        # Fit DBSCAN on scaled features
        cluster_predictions = self.dbscan_clustering.fit_predict(scaled_features)
