...
class HLTDBSCANAnomalyDetector(BaseClusteringDetector):
    ...
    def process(self,
        ...
        indices_not_nan = ~indices_nan #~ means inverse
        ...
        #cluster_predictions =\
            #self.dbscan_clustering.fit_predict(
                        #np.array(list(data_filtered)).reshape(-1,1)) #1D clustering with (data) each timestamp only found 1 clusters, reshape(-1,1) change 1 x 2600 to 2600 x 1

        num_clusters = len(set(cluster_predictions)) - (1 if -1 in cluster_predictions else 0)  # Ignore noise (-1)
        ...
        for subgroup, subgroup_bucket in subgroup_buckets.items():
            ...
            multimode_rack_bucket = multimode(cluster_predictions)#find most frequent element, return list if more than 1 element
            ...
            y_predicted = np.array([0 if cluster_prediction == largest_cluster_membership \
                                        else 1 for cluster_prediction in cluster_predictions], np.byte)#assume the most frequent are normal
            ...
            for machine_label, datapoint, y in zip(machine_labels_cluster, datapoints, y_predicted):
                ...
                if y == 1:
                    ...
                    elif np.isclose(datapoint, nan_fill_value):#Although NaNs were filtered earlier, if they were replaced with a fill value (nan_fill_value) during preprocessing, they won't be recognized as NaNs. So need to identify and drop these fill values, as they aren't outliers
